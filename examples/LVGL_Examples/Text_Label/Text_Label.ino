...
void lv_create_main_gui(void) {
  ...
  // - lv_label_create(parent) creates the label object
  // - lv_label_set_text(...) sets the displayed text
  // - lv_obj_center(...) positions the label in the middle of the screen
  lv_obj_t *label = lv_label_create(lv_scr_act());
  lv_label_set_text(label, "Hello, world!");
  lv_obj_center(label);

  // Style the label: create a style object and set the font
  // You can change the font (e.g. &lv_font_montserrat_18) if enabled in lv_conf.h
  ...
  // Customization tips:
  // - To change the text at runtime: lv_label_set_text(label, "New text");
  // - To reposition: lv_obj_align(label, LV_ALIGN_TOP_MID, 0, 20);
  // - To change color: lv_style_set_text_color(&label_style, lv_color_hex(0xFF0000));
}
